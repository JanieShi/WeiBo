...
- (void)_createTableView{
    ...
    _tableView = [[DetailTableView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain];
...
- (void)_loadMoreData{
    ...
    //设置max_id 分页加载
    DetailModel *cm = [self.data lastObject];
    if (cm == nil) {
        return;
    }
...
- (void)request:(SinaWeiboRequest *)request didFinishLoadingWithResult:(id)result
{
    ...
    for (NSDictionary *dataDic in array) {
        ...
        [modelArray addObject:detailModel];
    }
    ...
    if (request.tag == 100) {
        ...
        if (modelArray.count > 1) {
            ...
            [self.data addObjectsFromArray:modelArray];
        }else{
            return;
        }
    }
